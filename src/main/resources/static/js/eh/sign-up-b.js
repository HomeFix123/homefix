...
$(function() {
	//*********[ 사업자 회원가입 페이지 유효성 검사 정규식]*************************************************************/
	var RegexId = /^[a-zA-z0-9]{4,12}$/; //사업자 아이디는 영문 대소문자와 숫자 4~12자리
	var RegexCompany = /^[가-힣a-zA-Z0-9]{1,10}$/; //업체명 한글 및 영문자, 숫자 가능 1~10자리
	var RegexEmail = /^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i; //사업자 이메일
	var RegexPW = /^(?=.*[a-zA-z])(?=.*[0-9]).{4,10}$/; //4 ~ 10자 영문, 숫자 최소 한가지씩 조합
	var RegexName = /^[가-힣]+$/; //사업자명 한글만 가능
	var RegexTel = /^[0-9]{2,3}-[0-9]{3,4}-[0-9]{4}/;	//전화번호
	var RegexCompanyNum = /^(\d{3,3})+[-]+(\d{2,2})+[-]+(\d{5,5})$/;  //사업자번호

	var blank = "  필수 입력 사항입니다."; // 에러박스 문구

	//우편번호 찾기 클릭 이벤트(카카오 api )
	...
	$('#bs_btn_idCheak').click(function() {
		...
		$.ajax({
			...
			success: function(result) { // 중복 검사 후 나오는 결과 에러박스에 출력
				if (result == 'Y') {
