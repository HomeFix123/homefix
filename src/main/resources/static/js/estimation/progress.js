$(function(){
	...
	if(classify > 0){
		...
		li += "<a class='page-numbers' href='Progress?page="+ ((classify-1)*5+1)
		if(situation != null){
			li += "&sit=" + situation;
		}
		li += "'>"
	...
	for(let i = 1; i <= 5; i++){
		...
		if(classify*5+i == currentPage){
			...
			// 현재 페이지가 아닌 경우
			li += "<a class='page-numbers' href='Progress?page="+ (classify*5+i) +"'>"
		}
		li += classify*5+i
		li += "</a>"
		li += "</li>";
	...
	if(classify < parseInt((page-1)/5)){
		...
		li += "<a class='page-numbers' href='Progress?page="+ (classify*5+6) +"'>"
